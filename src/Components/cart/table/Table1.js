...
import { createOrder, createDuplicateOrder, getOrders } from "../../../Services/apiOrders";
...
const Table1 = ({ addressAdded }) => {
      ...
      try {
        ...
        if (error) {
          ...
          queryClient.setQueryData(["carts"], (oldData) =>
            oldData.map((item) =>
              item.id === productId
                ? { ...item, maxQuantity: newQuantity, totalPrice: totalPrice }
                : item
            )
          );
        }
  ...
  const { mutate: mutateCreateDuplicateOrder, isLoading: duplicateCreating } =
    useMutation({
      mutationFn: ({ userId, cartItems }) =>
        createDuplicateOrder(userId, cartItems),
      onSuccess: async () => {
  ...
  return (
    <>
      {closeReadyMessage && (
        <OrderPreparingMessage setCloseReadyMessage={setCloseReadyMessage} />
      )}
      <div className="table1">
        ...
        <div className="column3">
          {fetchCart?.length > 0 && user?.address && (
            <OrderButton
          ...
          {!user?.address && fetchCart?.length > 0 && (
            <button className="completeOrderBtn" onClick={handleBtnClick}>
              Complete Order
            </button>
          )}
